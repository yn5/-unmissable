			...
			<ThemedView style={styles.reminderContent}>
				...
				{isCompleted && (
					<ThemedText style={styles.completedAt}>
						Completed:{' '}
						{new Date(
							reminder.completedDates!.find(d =>
								isReminderCompletedOnDate({ ...reminder, completedDates: [d] }, date)
							)!
						).toLocaleString()}
					</ThemedText>
				)}
	...
	return (
		<Animated.View style={[styles.dayContainer, { width: SCREEN_WIDTH }]}>
			...
			<FlatList
				...
				renderItem={({ item }) => (
					<ReminderItem reminder={item} date={date} onComplete={() => onComplete(item.id, date)} />
				)}
				refreshControl={
					<RefreshControl
				...
				ListEmptyComponent={
					<ThemedText style={styles.emptyText}>No reminders for this day</ThemedText>
				}
